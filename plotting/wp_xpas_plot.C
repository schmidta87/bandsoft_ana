void wp_xpas_plot(TString inDat, TString inBac, TString inSim){
	...
	inTreeSim->Draw("tag[nleadindex]->getWp() >> wp_sim12","nHits[nleadindex]->getEdep()/1E4 > 12");
	inTreeSim->Draw("tag[nleadindex]->getWp() >> wp_sim19","nHits[nleadindex]->getEdep()/1E4 > 19");
