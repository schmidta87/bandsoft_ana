void xp(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_xp->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getXp_WP() >> xp_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("tag[nleadindex]->getXp_WP() >> xp_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("tag_smeared[nleadindex]->getXp_WP() >> xp_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		xp_dat[i]->Add(xp_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)xp_dat[i]->Integral() / xp_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		xp_sim[i]->Scale( full_simnorm );
		
		
		xp_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
