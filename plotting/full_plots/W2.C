void W2(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_W2->cd(i+1);
		inTreeDat->Draw(Form("sqrt(eHit->getW2()) >> W2_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("sqrt(eHit->getW2()) >> W2_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("sqrt(eHit_smeared->getW2()) >> W2_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		W2_dat[i]->Add(W2_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)W2_dat[i]->Integral() / W2_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		W2_sim[i]->Scale( full_simnorm );
		
		
		W2_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
