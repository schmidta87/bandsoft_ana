void phie(TString inDat, TString inBac, TString inSim){
	...
	for(int i = 0 ; i < 3 ; i++){
		phie_dat[i] = new TH1D(Form("phie_dat_%i",i),"",180,-180,180);
		phie_bac[i] = new TH1D(Form("phie_bac_%i",i),"",180,-180,180);
		phie_sim[i] = new TH1D(Form("phie_sim_%i",i),"",180,-180,180);
	}
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_phie->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getMomentumE().Phi()*180./TMath::Pi() >> phie_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("tag[nleadindex]->getMomentumE().Phi()*180./TMath::Pi() >> phie_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("tag_smeared[nleadindex]->getMomentumE().Phi()*180./TMath::Pi() >> phie_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		phie_dat[i]->Add(phie_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)phie_dat[i]->Integral() / phie_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		phie_sim[i]->Scale( full_simnorm );
		
		
		phie_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
