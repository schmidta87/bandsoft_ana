void omega(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_omega->cd(i+1);
		inTreeDat->Draw(Form("eHit->getOmega() >> omega_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("eHit->getOmega() >> omega_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("eHit_smeared->getOmega() >> omega_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		omega_dat[i]->Add(omega_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)omega_dat[i]->Integral() / omega_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		omega_sim[i]->Scale( full_simnorm );
		
		
		omega_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
