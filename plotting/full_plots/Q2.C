void Q2(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_Q2->cd(i+1);
		inTreeDat->Draw(Form("eHit->getQ2() >> Q2_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("eHit->getQ2() >> Q2_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("eHit_smeared->getQ2() >> Q2_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		Q2_dat[i]->Add(Q2_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)Q2_dat[i]->Integral() / Q2_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		Q2_sim[i]->Scale( full_simnorm );
		
		
		Q2_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
