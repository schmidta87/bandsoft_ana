void phin(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_phin->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getMomentumN().Phi()*180./TMath::Pi() >> phin_dat_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
		inTreeBac->Draw(Form("tag[nleadindex]->getMomentumN().Phi()*180./TMath::Pi() >> phin_bac_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
		inTreeSim->Draw(Form("tag[nleadindex]->getMomentumN().Phi()*180./TMath::Pi() >> phin_sim_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
