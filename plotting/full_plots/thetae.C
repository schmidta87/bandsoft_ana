void thetae(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_thetae->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getMomentumE().Theta()*180./TMath::Pi() >> thetae_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("tag[nleadindex]->getMomentumE().Theta()*180./TMath::Pi() >> thetae_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("tag_smeared[nleadindex]->getMomentumE().Theta()*180./TMath::Pi() >> thetae_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		thetae_dat[i]->Add(thetae_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)thetae_dat[i]->Integral() / thetae_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		thetae_sim[i]->Scale( full_simnorm );
		
		
		thetae_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
