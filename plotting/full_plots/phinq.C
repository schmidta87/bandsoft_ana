void phinq(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_phinq->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getPhiNQ()*180./TMath::Pi() >> phinq_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("tag[nleadindex]->getPhiNQ()*180./TMath::Pi() >> phinq_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("tag_smeared[nleadindex]->getPhiNQ()*180./TMath::Pi() >> phinq_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		phinq_dat[i]->Add(phinq_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)phinq_dat[i]->Integral() / phinq_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		phinq_sim[i]->Scale( full_simnorm );
		
		
		phinq_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
