void yn(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_yn->cd(i+1);
		inTreeDat->Draw(Form("nHits[nleadindex]->getDL().Y() >> yn_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("nHits[nleadindex]->getDL().Y() >> yn_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("nHits[nleadindex]->getDL().Y() >> yn_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		yn_dat[i]->Add(yn_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)yn_dat[i]->Integral() / yn_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		yn_sim[i]->Scale( full_simnorm );
		
		
		yn_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
