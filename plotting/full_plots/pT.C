void pT(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 1 ; i++){
		...
		c_pT->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getPt().Mag() >> pT_dat_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
		inTreeBac->Draw(Form("tag[nleadindex]->getPt().Mag() >> pT_bac_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
		inTreeSim->Draw(Form("tag[nleadindex]->getPt().Mag() >> pT_sim_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
