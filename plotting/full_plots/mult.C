void mult(TString inDat, TString inBac, TString inSim){
	...
	TH1D ** mult_dat = new TH1D*[3];
	TH1D ** mult_bac = new TH1D*[3];
	TH1D ** mult_sim = new TH1D*[3];
	for(int i = 0 ; i < 3 ; i++){
		mult_dat[i] = new TH1D(Form("mult_dat_%i",i),"",10,0,10);
		mult_bac[i] = new TH1D(Form("mult_bac_%i",i),"",10,0,10);
		mult_sim[i] = new TH1D(Form("mult_sim_%i",i),"",10,0,10);
	}

	// Draw the full mult distribution
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_mult->cd(i+1);
		inTreeDat->Draw(Form("nMult >> mult_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("nMult >> mult_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("nMult >> mult_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		mult_dat[i]->Add(mult_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)mult_dat[i]->Integral() / mult_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		mult_sim[i]->Scale( full_simnorm );
		
		
		mult_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
		label1D(mult_dat[i],mult_sim[i],"Multiplicity","Counts");

		c_mult->cd(4+i);
		label1D_ratio(mult_dat[i],mult_sim[i],"Multiplicity","Data/Sim",0,3);
	}


	c_mult->SaveAs("full_mult.pdf");
