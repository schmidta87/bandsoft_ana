void wp(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_wp->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getWp() >> wp_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("tag[nleadindex]->getWp() >> wp_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("tag_smeared[nleadindex]->getWp() >> wp_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		wp_dat[i]->Add(wp_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)wp_dat[i]->Integral() / wp_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		wp_sim[i]->Scale( full_simnorm );
		
		
		wp_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
