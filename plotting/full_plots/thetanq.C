void thetanq(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_thetanq->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getThetaNQ()*180./TMath::Pi() >> thetanq_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("tag[nleadindex]->getThetaNQ()*180./TMath::Pi() >> thetanq_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("tag_smeared[nleadindex]->getThetaNQ()*180./TMath::Pi() >> thetanq_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		thetanq_dat[i]->Add(thetanq_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)thetanq_dat[i]->Integral() / thetanq_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		thetanq_sim[i]->Scale( full_simnorm );
		
		
		thetanq_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
