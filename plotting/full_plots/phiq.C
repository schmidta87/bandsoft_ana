void phiq(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_phiq->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getMomentumQ().Phi()*180./TMath::Pi() >> phiq_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("tag[nleadindex]->getMomentumQ().Phi()*180./TMath::Pi() >> phiq_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("tag_smeared[nleadindex]->getMomentumQ().Phi()*180./TMath::Pi() >> phiq_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		phiq_dat[i]->Add(phiq_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)phiq_dat[i]->Integral() / phiq_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		phiq_sim[i]->Scale( full_simnorm );
		
		
		phiq_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
