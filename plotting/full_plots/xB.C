void xB(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_xB->cd(i+1);
		inTreeDat->Draw(Form("eHit->getXb() >> xB_dat_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
		inTreeBac->Draw(Form("eHit->getXb() >> xB_bac_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
		inTreeSim->Draw(Form("eHit->getXb() >> xB_sim_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut);
