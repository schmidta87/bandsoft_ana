void xB(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_xB->cd(i+1);
		inTreeDat->Draw(Form("eHit->getXb() >> xB_dat_%i",i), pNcut && pTcut[i]);
		inTreeBac->Draw(Form("eHit->getXb() >> xB_bac_%i",i), pNcut && pTcut[i]);
		inTreeSim->Draw(Form("eHit_smeared->getXb() >> xB_sim_%i",i), pNcut_sim && pTcut_sim[i]);

		// Background subraction
		xB_dat[i]->Add(xB_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)xB_dat[i]->Integral() / xB_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		xB_sim[i]->Scale( full_simnorm );
		
		
		xB_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
