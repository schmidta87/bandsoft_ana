void virt(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_virt->cd(i+1);
		inTreeDat->Draw(Form("( pow(1.8756 - sqrt( pow(tag[nleadindex]->getMomentumN().Mag(),2) + pow(0.9396 ,2) ) ,2) - pow( tag[nleadindex]->getMomentumN().Mag() , 2 ) - pow(0.93827,2) )/pow(0.93827,2) >> virt_dat_%i",i),pNcut && pTcut[i]);
		inTreeBac->Draw(Form("( pow(1.8756 - sqrt( pow(tag[nleadindex]->getMomentumN().Mag(),2) + pow(0.9396 ,2) ) ,2) - pow( tag[nleadindex]->getMomentumN().Mag() , 2 ) - pow(0.93827,2) )/pow(0.93827,2) >> virt_bac_%i",i),pNcut && pTcut[i]);
		inTreeSim->Draw(Form("( pow(1.8756 - sqrt( pow(tag_smeared[nleadindex]->getMomentumN().Mag(),2) + pow(0.9396 ,2) ) ,2) - pow( tag_smeared[nleadindex]->getMomentumN().Mag() , 2 ) - pow(0.93827,2) )/pow(0.93827,2) >> virt_sim_%i",i),pNcut_sim && pTcut_sim[i]);

		// Background subraction
		virt_dat[i]->Add(virt_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)virt_dat[i]->Integral() / virt_sim[i]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		virt_sim[i]->Scale( full_simnorm );
		
		
		virt_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",full_simnorm));
