void asBins_wpBins_xp_ratios(TString inDat, TString inBac, TString inSim){
	...
	TH1D *** xp_dat = new TH1D**[nAs_bins];
	TH1D *** xp_bac = new TH1D**[nAs_bins];
	TH1D *** xp_sim = new TH1D**[nAs_bins];
	for( int bin = 0 ; bin < nAs_bins ; bin++ ){
		xp_dat[bin] = new TH1D*[nWp_coarse_bins];
		xp_bac[bin] = new TH1D*[nWp_coarse_bins];
		xp_sim[bin] = new TH1D*[nWp_coarse_bins];
		for( int wp_bin = 0 ; wp_bin < nWp_coarse_bins ; wp_bin++ ){
			xp_dat[bin][wp_bin] = new TH1D(Form("h1_xp_dat_%i_%i",bin,wp_bin),"",nXp_bins,Xp_min,Xp_max);
			xp_bac[bin][wp_bin] = new TH1D(Form("h1_xp_bac_%i_%i",bin,wp_bin),"",nXp_bins,Xp_min,Xp_max);
			xp_sim[bin][wp_bin] = new TH1D(Form("h1_xp_sim_%i_%i",bin,wp_bin),"",nXp_bins,Xp_min,Xp_max);
		}
	}
	...
	TCanvas * c1_as_wp_xp_tag = new TCanvas("c1_as_wp_xp_tag","",800,600);
	TCanvas * c1_as_wp_xp_ratio = new TCanvas("c1_as_wp_xp_ratio","",800,600);
	c1_as_wp_xp_tag->Divide(nWp_coarse_bins,	nAs_bins);
	c1_as_wp_xp_ratio->Divide(nWp_coarse_bins,	nAs_bins);
	for( int bin = 0 ; bin < nAs_bins ; bin++ ){
		...
		// Now loop over the W' bins we want:
		for( int wp_bin = 0 ; wp_bin < nWp_coarse_bins ; wp_bin++ ){
			double this_min_wp = Wp_min + wp_bin*(Wp_max - Wp_min)/nWp_coarse_bins;
			double this_max_wp = Wp_min + (wp_bin+1)*(Wp_max - Wp_min)/nWp_coarse_bins;
			TCut this_wp_cut = Form("tag[nleadindex]->getWp() > %f && tag[nleadindex]->getWp() < %f",this_min_wp,this_max_wp);
		
			//	set title of histograms
			TString current_title = Form("%.2f < As < %.2f, %.2f < W' < %.2f",this_min_as,this_max_as,this_min_wp,this_max_wp);
			xp_dat[bin][wp_bin]->SetTitle( current_title );
			xp_sim[bin][wp_bin]->SetTitle( current_title );


			// 	fill the histograms of X' for given As bin, W' bin
			c1_as_wp_xp_ratio->cd(wp_bin+1 + bin*nWp_coarse_bins);
			c1_as_wp_xp_tag->cd(wp_bin+1 + bin*nWp_coarse_bins);
			inTreeDat->Draw(Form("tag[nleadindex]->getXp() >> h1_xp_dat_%i_%i",bin,wp_bin), this_as_cut && this_wp_cut && glob_cuts );
			inTreeBac->Draw(Form("tag[nleadindex]->getXp() >> h1_xp_bac_%i_%i",bin,wp_bin), this_as_cut && this_wp_cut && glob_cuts );
			inTreeSim->Draw(Form("tag[nleadindex]->getXp() >> h1_xp_sim_%i_%i",bin,wp_bin), this_as_cut && this_wp_cut && glob_cuts );

			if( xp_dat[bin][wp_bin]->Integral() == 0 || xp_bac[bin][wp_bin]->Integral() == 0 || xp_sim[bin][wp_bin]->Integral() == 0 ){ 
				c1_as_wp_xp_tag->cd(wp_bin+1 + bin*nWp_coarse_bins);
				c1_as_wp_xp_ratio->cd(wp_bin+1 + bin*nWp_coarse_bins);
				continue;
			}
			...
			xp_dat[bin][wp_bin]->Add( xp_bac[bin][wp_bin] , -1 );
			//	do simulation scaling
			xp_sim[bin][wp_bin]->Scale( simnorm );

			
			c1_as_wp_xp_tag->cd(wp_bin+1 + bin*nWp_coarse_bins);
			label1D( xp_dat[bin][wp_bin] , xp_sim[bin][wp_bin], "x'","Counts [a.u.]");

			c1_as_wp_xp_ratio->cd(wp_bin+1 + bin*nWp_coarse_bins);
			label1D_ratio( xp_dat[bin][wp_bin] , xp_sim[bin][wp_bin], "x'","Data/Sim",0,2);
		}
	}
	c1_as_wp_xp_tag		->SaveAs("asBins_wpBins_xp.pdf");
	c1_as_wp_xp_ratio	->SaveAs("asBins_wpBins_xp_ratio.pdf");
