void asBins_xpBins_Q2_ratios(TString inDat, TString inBac, TString inSim){
	...
	for( int bin = 0 ; bin < nAs_bins ; bin++ ){
		Q2_dat[bin] = new TH1D*[nXp_coarse_bins];
		Q2_bac[bin] = new TH1D*[nXp_coarse_bins];
		Q2_sim[bin] = new TH1D*[nXp_coarse_bins];
		for( int xp_bin = 0 ; xp_bin < nXp_coarse_bins ; xp_bin++ ){
			Q2_dat[bin][xp_bin] = new TH1D(Form("h1_Q2_dat_%i_%i",bin,xp_bin),"",nQ2_bins,Q2_min,Q2_max);
			Q2_bac[bin][xp_bin] = new TH1D(Form("h1_Q2_bac_%i_%i",bin,xp_bin),"",nQ2_bins,Q2_min,Q2_max);
			Q2_sim[bin][xp_bin] = new TH1D(Form("h1_Q2_sim_%i_%i",bin,xp_bin),"",nQ2_bins,Q2_min,Q2_max);
		}
	}
	...
	TCanvas * c1_as_xp_Q2_tag = new TCanvas("c1_as_xp_Q2_tag","",800,600);
	TCanvas * c1_as_xp_Q2_ratio = new TCanvas("c1_as_xp_Q2_ratio","",800,600);
	c1_as_xp_Q2_tag->Divide(nXp_coarse_bins,	nAs_bins);
	c1_as_xp_Q2_ratio->Divide(nXp_coarse_bins,	nAs_bins);
	for( int bin = 0 ; bin < nAs_bins ; bin++ ){
		...
		for( int xp_bin = 0 ; xp_bin < nXp_coarse_bins ; xp_bin++ ){
			...
			Q2_dat[bin][xp_bin]->SetTitle( current_title );
			Q2_sim[bin][xp_bin]->SetTitle( current_title );


			// 	fill the histograms of X' for given As bin, W' bin
			c1_as_xp_Q2_ratio->cd(xp_bin+1 + bin*nXp_coarse_bins);
			c1_as_xp_Q2_tag->cd(xp_bin+1 + bin*nXp_coarse_bins);
			inTreeDat->Draw(Form("eHit->getQ2() >> h1_Q2_dat_%i_%i",bin,xp_bin), this_as_cut && this_xp_cut && glob_cuts );
			inTreeBac->Draw(Form("eHit->getQ2() >> h1_Q2_bac_%i_%i",bin,xp_bin), this_as_cut && this_xp_cut && glob_cuts );
			inTreeSim->Draw(Form("eHit->getQ2() >> h1_Q2_sim_%i_%i",bin,xp_bin), this_as_cut && this_xp_cut && glob_cuts );

			if( Q2_dat[bin][xp_bin]->Integral() == 0 || Q2_bac[bin][xp_bin]->Integral() == 0 || Q2_sim[bin][xp_bin]->Integral() == 0 ){ 
				c1_as_xp_Q2_tag->cd(xp_bin+1 + bin*nXp_coarse_bins);
				c1_as_xp_Q2_ratio->cd(xp_bin+1 + bin*nXp_coarse_bins);
				continue;
			}
			...
			Q2_dat[bin][xp_bin]->Add( Q2_bac[bin][xp_bin] , -1 );
			//	do simulation scaling
			Q2_sim[bin][xp_bin]->Scale( simnorm );

			
			c1_as_xp_Q2_tag->cd(xp_bin+1 + bin*nXp_coarse_bins);
			label1D( Q2_dat[bin][xp_bin] , Q2_sim[bin][xp_bin], "Q^{2}","Counts [a.u.]");

			c1_as_xp_Q2_ratio->cd(xp_bin+1 + bin*nXp_coarse_bins);
			label1D_ratio( Q2_dat[bin][xp_bin] , Q2_sim[bin][xp_bin], "Q^{2}","Data/Sim",0,2);
		}
	}
	c1_as_xp_Q2_tag		->SaveAs("asBins_xpBins_Q2.pdf");
	c1_as_xp_Q2_ratio	->SaveAs("asBins_xpBins_Q2_ratio.pdf");
